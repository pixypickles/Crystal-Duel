...
const W=1280,H=720,TOP=82,BOTTOM=680,LANE_H=(BOTTOM-TOP)/5,MAX=3,CHARGE=650;
const PX=[205,1075],CX=[72,1208],WX=[285,995];
const speed={a:330,b:500,c:720};
let S,raf,last=0;
function player(side){return{side,lane:2,hp:20,guard:false,wall:null,charge:null,stun:0,guardTap:-9999,aiShot:0,aiMove:0}}
...
function count(side){return S.bullets.reduce((n,b)=>n+(b.side===side&&!b.dead),0)}
...
function attackDown(side,t){if(!S.running)return;const p=S.players[side],now=performance.now();if(now<p.stun||p.charge||count(side)>=MAX)return;p.charge={t,start:now}}
function attackUp(side,t,forcedHeld){const p=S.players[side];if(!p.charge||p.charge.t!==t)return;const now=performance.now();if(!S.running||now<p.stun||count(side)>=MAX){p.charge=null;return}const held=forcedHeld??(now-p.charge.start),charged=held>=CHARGE,dir=side? -1:1;S.bullets.push({side,lane:p.lane,x:PX[side]+dir*45,y:ly(p.lane),vx:speed[t]*dir,damage:charged?5:3,r:charged?24:13,dead:false,turn:p.lane===0||p.lane===4});p.charge=null;spark(PX[side],ly(p.lane),side?'#ff88d7':'#77efff',8)}
...
function syncHud(){for(const [p,bar,txt] of [[S.players[0],pHp,pHpText],[S.players[1],eHp,eHpText]]){bar.style.width=(p.hp/20*100)+'%';txt.textContent=p.hp}}
...
function ai(now){const a=S.players[1],you=S.players[0];if(now<a.stun)return;
 if(now>a.aiMove){const danger=S.bullets.find(b=>b.side===0&&!b.dead&&b.x>650&&Math.abs(b.y-ly(a.lane))<35);if(danger&&Math.random()<.65){if(Math.random()<.45)guardDown(1);else move(1,a.lane<2?1:-1)}else{const target=Math.random()<.55?you.lane:Math.floor(Math.random()*5);move(1,Math.sign(target-a.lane))}a.aiMove=now+350+Math.random()*500}
 if(now>a.aiShot&&count(1)<MAX){const types=['a','b','c'],t=types[Math.floor(Math.random()*3)];attackDown(1,t);attackUp(1,t,Math.random()<.28?800:120);a.aiShot=now+500+Math.random()*700}
...
function update(dt,now){if(!S.running)return;S.time+=dt;ai(now);
 ...
 for(let i=0;i<S.bullets.length;i++)for(let j=i+1;j<S.bullets.length;j++){const a=S.bullets[i],b=S.bullets[j];if(a.dead||b.dead||a.side===b.side)continue;if(Math.hypot(a.x-b.x,a.y-b.y)<a.r+b.r){if(a.damage===b.damage){a.dead=b.dead=true}else{const hi=a.damage>b.damage?a:b,lo=hi===a?b:a;lo.dead=true;hi.damage=2;hi.r=10}spark((a.x+b.x)/2,(a.y+b.y)/2,'#dffcff',14)}}
...
function start(){reset(true);overlay.classList.remove('show');result.classList.remove('show')}
