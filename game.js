...
function ai(now){const a=S.players[1],you=S.players[0];if(now<a.stun)return;
 ...
 if(now>a.aiShot){const available=['a','b','c'].filter(t=>!hasType(1,t));if(available.length){const t=available[Math.floor(Math.random()*available.length)];attackDown(1,t);attackUp(1,t,Math.random()<.28?800:120)}a.aiShot=now+500+Math.random()*700}
