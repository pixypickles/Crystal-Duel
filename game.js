(() => {
  ...
  function releaseAttack(side, type) {
    ...
    state.bullets.push({
      side, lane: p.lane, x: PLAYER_X[side] + direction * 42, y: laneY(p.lane),
  ...
  function update(dt, now) {
    ...
    for (const b of state.bullets) {
      ...
      if (wall && wall.alive && wall.lane === b.lane && Math.abs(b.x - WALL_X[defender.side]) < Math.abs(b.vx * dt) + 18) {
        wall.alive = false;
        b.dead = true;
      ...
      if (defender.lane === b.lane && Math.abs(b.x - PLAYER_X[defender.side]) < 34) {
        hitPlayer(defender, b, now);
        continue;
      }

      const crystalLane = b.lane >= 1 && b.lane <= 3;
    ...
    for (let i = 0; i < state.bullets.length; i++) {
      ...
      for (let j = i + 1; j < state.bullets.length; j++) {
        ...
        if (b.dead || a.side === b.side || a.lane !== b.lane) continue;
  ...
  document.getElementById('startBtn').addEventListener('click', () => {
    startOverlay.classList.remove('show'); state.running = true; state.last = performance.now();
  });
